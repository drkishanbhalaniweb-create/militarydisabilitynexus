...
import { mapContactToZohoLead } from '../../src/lib/zohoLeadMapper';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      const leadPayload = mapContactToZohoLead(preparedContact, attribution);
      const zohoResult = await upsertLead(leadPayload);
      if (!zohoResult.success && !zohoResult.skipped) {
        console.warn('[ZohoCRM] Contact sync warning:', zohoResult.error);
      }
    } catch (zohoError) {
